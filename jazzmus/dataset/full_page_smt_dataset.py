"""
SMT Dataset for full-page jazz leadsheet recognition.
Mirrors the system-level GrandStaffDataset structure but for full-page data.
"""
import re
import cv2
import gin
...
def load_set(
    ...
    reduce_ratio=1.0,
    fixed_size=None,
    fixed_img_height=800,
    max_fix_img_width=1302,
    include_synthetic=False,
):
    """Load full-page image set from fold-based split files."""
    x = []
    y = []
    paths = []

    # Read from the given split file
    with open(f"{dataset}/splits/{split}_{fold}.txt") as f:
        lines = f.readlines()
        kern_samples = [line.split(" ")[1].strip() for line in lines]
        img_samples = [line.split(" ")[0].strip() for line in lines]
    
    # print(f"Number of full-page images in {split} split: {len(img_samples)}")



    if include_synthetic and split == "train":
        img_samples_stems = [Path(img).stem for img in img_samples]
        kern_folder = Path("data/jazzmus_fullpage_syn/gt")
        counter = 0
        # add the pieces from data/la_result to the dataset
        assert Path("data/jazzmus_fullpage_syn").exists(), "data/la_result does not exist. Create synthetic first!"
        # take only the pieces that are in the train split
        for piece in Path("data/jazzmus_fullpage_syn").rglob("*.jpg"):
    ...
    print(f"Number of full-pages in split: {split}: {len(img_samples)}")
    ...
    for img_sample, kern_sample in progress.track(zip(img_samples, kern_samples)):
        ...
        if img_raw is None:
            print(f"Warning: Could not load image {img_sample}")
            continue
...
def batch_preparation_img2seq(data):
    """Prepare batch for full-page SMT training."""
    images = [sample[0] for sample in data]
    dec_in = [sample[1] for sample in data]
    gt = [sample[2] for sample in data]
    paths = [sample[3] for sample in data]

    # Enforce minimum batch dimensions for full-page (different aspect ratio than system-level)
    max_image_width = max(150, max([img.shape[2] for img in images]))
...
@gin.configurable
class GrandStaffFullPage(OMRIMG2SEQDataset):
    def __init__(
        self,
        data_path,
        split,
...
@gin.configurable
class FullPageGrandStaffDataset(LightningDataModule):
    def __init__(
        ...
        self.fold = fold
        self.train_set = GrandStaffFullPage(
            data_path=self.data_path,
            split="train",
            augment=True,
            fold=self.fold,
        )
        self.val_set = GrandStaffFullPage(

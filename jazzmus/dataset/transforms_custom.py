...
class ElasticDistortion:
    ...
    def __call__(self, x):
        ...
        dimensions = []
        shift = [
            [(0, 0) for x in range(horizontal_tiles)] for y in range(vertical_tiles)
        ]
        ...
        return x.transform(
            x.size, Image.MESH, self.generated_mesh, resample=Image.BICUBIC
        )
...
class RandomTransform:
    ...
    def __call__(self, x):
        ...
        x = stf.warp(
            np.array(x), tform, output_shape=output_shape, cval=255, preserve_range=True
        )

...
class PositionalEncoding2D(nn.Module):
    def __init__(self, dim, h_max, w_max):
        ...
        # Ensure `dim` matches the model’s expected embedding size
    ...
    def forward(self, x):
        ...
        if x.size(1) != self.dim:
            raise ValueError(
                f"Expected input with {self.dim} channels, but got {x.size(1)}"
            )
...
class MHA(nn.Module):
    def __init__(
        self, embedding_dim, num_heads=None, dropout=0, proj_value=True
    ) -> None:
        super().__init__()
    ...
    def forward(
        self, query, key, value, key_pad_mask=None, attn_mask=None, get_weights=True
    ):
...
class DecoderStack(nn.Module):
    def __init__(self, num_dec_layers, d_model, dim_ff) -> None:
        ...
        self.layers = nn.ModuleList(
            [
                DecoderLayer(d_model=d_model, dim_ff=dim_ff)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.decoder = DecoderStack(
            num_dec_layers=n_layers, d_model=d_model, dim_ff=dim_ff
        )

        self.embedding = nn.Embedding(
            num_embeddings=out_categories, embedding_dim=d_model
        )
...
class SMTModelForCausalLM(PreTrainedModel):
    ...
    def __init__(self, config: SMTConfig):
        ...
        self.positional_2D = PositionalEncoding2D(
            config.d_model, config.maxh, config.maxw
        )
    ...
    def forward_decoder(self, encoder_output, y_pred):
        ...
        features = torch.flatten(encoder_output, start_dim=2, end_dim=3).permute(
            2, 0, 1
        )
        enhanced_features = features
        enhanced_features = torch.flatten(pos_features, start_dim=2, end_dim=3).permute(
            2, 0, 1
        )
    ...
    def predict(self, input, convert_to_str=False):
        predicted_sequence = (
            torch.from_numpy(np.asarray([self.w2i["<bos>"]]))
            .to(input.device)
            .unsqueeze(0)
        )
